...
void tgl_prng_seed (struct tgl_state *TLS, const char *password_filename, int password_length) {
  ...
  TGLCM.RAND_add (&T, sizeof (T), 4.0);
#ifdef HAVE_RDTSC
  unsigned long long r = rdtsc ();
  TGLCM.RAND_add (&r, 8, 4.0);
#endif
  unsigned short p = getpid ();
  TGLCM.RAND_add (&p, sizeof (p), 0.0);
  p = getppid ();
  TGLCM.RAND_add (&p, sizeof (p), 0.0);
  unsigned char rb[32];
  int s = get_random_bytes (TLS, rb, 32);
  if (s > 0) {
    TGLCM.RAND_add (rb, s, s);
  }
  ...
  if (password_filename && password_length > 0) {
    ...
    if (fd < 0) {
      ...
      if (l < 0) {
        ...
        TGLCM.RAND_add (a, l, l);
      }
      close (fd);
      tfree_secure (a, password_length);
    }
  }
  TLS->BN_ctx = TGLCM.BN_CTX_new ();
...
int tgl_serialize_bignum (TGLC_BIGNUM  *b, char *buffer, int maxlen) {
  ...
  int l = TGLCM.BN_bn2bin (b, (unsigned char *)buffer);
  assert (l == itslen);
  buffer += l;
  while (pad --> 0) {
    *buffer++ = 0;
...
long long tgl_do_compute_rsa_key_fingerprint (TGLC_RSA *key) {
  ...
  TGLCM.SHA1 ((unsigned char *)tempbuff, l1 + l2, sha);
...
int tgl_fetch_bignum (TGLC_BIGNUM *x) {
  int l = prefetch_strlen ();
  if (l < 0) {
    return l;
  }
  char *str = fetch_str (l);
  assert (TGLCM.BN_bin2bn ((unsigned char *) str, l, x) == x);
  return l;
}

int tgl_pad_rsa_encrypt (struct tgl_state *TLS, char *from, int from_len, char *to, int size, TGLC_BIGNUM *N, TGLC_BIGNUM *E) {
  int pad = (255000 - from_len - 32) % 255 + 32;
  int chunks = (from_len + pad) / 255;
  int bits = TGLCM.BN_num_bits (N);
...
int tgl_pad_rsa_decrypt (struct tgl_state *TLS, char *from, int from_len, char *to, int size, TGLC_BIGNUM *N, TGLC_BIGNUM *D) {
...
void tgl_init_aes_auth (char auth_key[192], char msg_key[16], int encrypt) {
  ...
  //  sha1_a = TGLCM.SHA1 (msg_key + substr (auth_key, 0, 32));
  //  sha1_b = TGLCM.SHA1 (substr (auth_key, 32, 16) + msg_key + substr (auth_key, 48, 16));
  //  sha1_с = TGLCM.SHA1 (substr (auth_key, 64, 32) + msg_key);
  //  sha1_d = TGLCM.SHA1 (msg_key + substr (auth_key, 96, 32));
...
int tgl_pad_aes_encrypt (char *from, int from_len, char *to, int size) {
  ...
  if (from_len < padded_size) {
    assert (TGLCM.RAND_pseudo_bytes ((unsigned char *) from + from_len, padded_size - from_len) >= 0);
  }
  TGLCM.AES_ige_encrypt ((unsigned char *) from, (unsigned char *) to, padded_size, &aes_key, aes_iv, AES_ENCRYPT);
  return padded_size;
}
...
int tgl_pad_aes_decrypt (char *from, int from_len, char *to, int size) {
  ...
  TGLCM.AES_ige_encrypt ((unsigned char *) from, (unsigned char *) to, from_len, &aes_key, aes_iv, AES_DECRYPT);
  return from_len;
}
